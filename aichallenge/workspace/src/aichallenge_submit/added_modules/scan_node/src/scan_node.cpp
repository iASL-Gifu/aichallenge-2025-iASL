...
// =================================================================
// ScanSimulatorNode クラスの実装
// =================================================================

ScanSimulatorNode::ScanSimulatorNode() : Node("scan_simulator_node")
{
    // (この関数の中身は変更なし)
    this->set_parameter(rclcpp::Parameter("use_sim_time", true));
    RCLCPP_INFO(this->get_logger(), "2D Scan Simulator Node starting...");
    ...
    timer_ = this->create_wall_timer(timer_period, std::bind(&ScanSimulatorNode::timer_callback, this));
    RCLCPP_INFO(this->get_logger(), "Timer started with a frequency of %.2f Hz.", timer_hz_);
}

void ScanSimulatorNode::declare_and_get_params()
{
...
void ScanSimulatorNode::load_walls_from_csv()
{
...
void ScanSimulatorNode::timer_callback()
{
...
void ScanSimulatorNode::run_simulation(const PoseWithCovarianceStamped::ConstSharedPtr& pose_msg)
{
    // --- ロボットの自己位置にもオフセットを適用 ---
    Point2D robot_pos = {
        ...
        if (min_dist_sq < max_range_ * max_range_) {
            ...
            if (distance >= range_min_) {
                ...
                if (debug_ && closest_intersection) {
                    ...
                    // ヒットポイントは既にオフセット後の座標系で計算されている
                    p.x = closest_intersection->x;
                    p.y = closest_intersection->y;
                    // Z座標はオフセットを適用しない（通常は0か、ロボットの高さに合わせる）
...
std::optional<Point2D> ScanSimulatorNode::get_line_segment_intersection(Point2D p1, Point2D p2, Point2D p3, Point2D p4)
{
...
void ScanSimulatorNode::publish_wall_markers()
{
    Marker wall_marker;
