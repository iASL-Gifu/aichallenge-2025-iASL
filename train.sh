...
# ★ 変更点 1: 共有メモリのサイズを変数として定義
SHM_SIZE="2g" # 2GBに設定（環境に合わせて1gなどに調整してください）
...
# ★ 変更点 2: rockerコマンドに --shm-size オプションを追加
rocker ${opts} --shm-size=${SHM_SIZE} --x11 --devices /dev/dri --user --net host --privileged --name "aichallenge-2025-$(date "+%Y-%m-%d-%H-%M-%S")" --volume ${volume} -- "aichallenge-2025-${target}-${USER}" 2>&1 | tee "$LOG_FILE"